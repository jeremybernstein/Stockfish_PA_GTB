...
namespace {
    ...
    *ponderMove = bestMove = easyMove = skillBest = skillPonder = MOVE_NONE;
        ...
        && pos.total_piece_count() <= MaxEgtbPieces)
    {
      ...
      if (pos.tb_hits_root())
      {
        ...
        for (int i = 0; i < Rml.size() && i < MultiPV; i++) {
          int seldepth = 1;
          for (Move* m = Rml[i].pv; *m != MOVE_NONE; m++)
            seldepth++;
          cout << Rml[i].pv_info_to_uci(pos, 1, seldepth, -VALUE_INFINITE, VALUE_INFINITE, i) << endl;
        }
    ...
    while (!StopRequest && ++depth <= PLY_MAX && (!Limits.maxDepth || depth <= Limits.maxDepth))
    {
        ...
        if (MultiPV == 1 && depth >= 5 && abs(bestValues[depth - 1]) < VALUE_KNOWN_WIN)
        {
            ...
            aspirationDelta = (aspirationDelta + 7) / GrainSize * GrainSize; // Round to match grainSize
  ...
  Value root_tb_mainline(Position& pos, RootMoveList& rm) {
    ...
    if (Limits.useTimeManagement() && current_search_time() > TimeMgr.available_time() / 16)
        return VALUE_NONE;
    ...
    while (1) {
        ...
        Rml.init(pos, searchMoves);
        Rml.extract_pv_from_tb(pos);
        // maybe we can eliminate the pos.is_draw() check under certain circumstances
        if (Rml.size() && index <= PLY_MAX && !pos.is_draw()) {
            rm[0].pv[index] = Rml[0].pv[0];
        } else break;
        // TODO: is this lame? is TM really a concern here?
        if (Limits.useTimeManagement() && current_search_time() > TimeMgr.available_time() / 16)
            break;
    }
  ...
  Value search(Position& pos, SearchStack* ss, Value alpha, Value beta, Depth depth) {
    ...
    if (   UseGaviotaTb 
        && !ProbeOnlyAtRoot
        && pos.total_piece_count() <= MaxEgtbPieces
        && ((tbValue = attempt_probe_egtb(pos, true, ss->ply, depth, alpha, beta, (Limits.useTimeManagement() && current_search_time() > TimeMgr.available_time() / 16))) != VALUE_NONE) )
    {
/*        // TODO: is this really necessary? I think it may be automatically done.
        if (tbValue == VALUE_KNOWN_WIN)
            ...
            TT.store(pos.get_key(), value_to_tt(tbValue, ss->ply), VALUE_TYPE_EXACT, depth, MOVE_NONE, tbValue, VALUE_ZERO);
*/
        return tbValue;
    }
#endif
...
split_point_start: // At split points actual search starts from here
  ...
  Value qsearch(Position& pos, SearchStack* ss, Value alpha, Value beta, Depth depth) {
    ...
    if (   UseGaviotaTb
        && pos.total_piece_count() <= MaxEgtbPieces
        && ((tbValue = attempt_probe_egtb(pos, true, ss->ply, depth, alpha, beta, false)) != VALUE_NONE) )
    {
...
*/
  ...
  Value attempt_probe_egtb(Position& pos, bool pvNode, int ply, Depth depth, Value alpha, Value beta, bool forcesoft)
  {
      ...
      exact = (alpha < -VALUE_KNOWN_WIN || beta > VALUE_KNOWN_WIN);
...
#ifdef USE_EGTB
  void RootMoveList::extract_pv_from_tb(Position& pos) {
    ...
    for (int i = 0; i < size(); i++)
    {
      pos.do_move(at(i).pv[0], st);
      at(i).pv_score = -qsearch<PV>(pos, ss+1, -VALUE_INFINITE, VALUE_INFINITE, DEPTH_ZERO);
      pos.undo_move(at(i).pv[0]);
    }
    sort();
  }
#endif    
